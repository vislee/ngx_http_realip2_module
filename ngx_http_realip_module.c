...
ngx_http_realip_set_addr(ngx_http_request_t *r, ngx_addr_t *addr)
{
    ...
    ctx->type = rlcf? rlcf->type: NGX_HTTP_REALIP_LUA;
...
int
ngx_http_lua_ffi_realip_set_addr(ngx_http_request_t *r,
    const char *ip, size_t len, char **errmsg)
{
    ngx_int_t   rc;
    ngx_addr_t  addr;

    rc = ngx_parse_addr(r->connection->pool, &addr, (u_char *)ip, len);
    if (rc != NGX_OK) {
        *errmsg = "failed to ngx_parse_addr";
        return NGX_ERROR;
    }
